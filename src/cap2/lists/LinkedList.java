...
public class LinkedList {
	...
	public Mp3 removeNode_byPos(int pos) {
		//Instance the result param
		Mp3 result = null;
		// Call the node
		Node tmp = this.head;
		//Create the posTmp var
		int posTmp = 0;
		while(posTmp <= pos) {
			/*Don't do anything, only increment to posTmp*/
			posTmp = posTmp + 1;
		}
		/*In the following line, we execute a erasing in the correct node*/
		tmp.setDato(null);;
		
		if(tmp.getDato().getTrackNo()=="") {
			result = null;
			return result;
		}else {
			return result;
